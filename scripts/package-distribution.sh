...
generate_checksums() {
  local package_dir="$1"
  resolve_checksum_command
  (cd "$package_dir" && "${CHECKSUM_CMD[@]}" docker-compose.yml .env.example runner-agent.env.example runner-agent.sh runner-agent-smoke.sh live-kernel-smoke.sh cyanrex-release install-smoke.sh LICENSE README.md README-en.md README-zh-CN.md README-docker.md README-DEPLOY.md manifest.env release-metadata.json deploy.sh run.sh stop.sh cyanrex-images.tar > checksums.sha256)
}
resolve_version
require_cmd tar
require_cmd node
