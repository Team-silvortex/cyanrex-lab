...
点击学生旁的“查看尝试”可以复盘最近 20 次提交。每条记录会显示后端阶段、运行与挂载结果、
自动反馈及学生实际提交的源码，便于区分编译失败、verifier 失败和挂载失败，并进一步检查
学生的推理过程。复盘接口仅允许教师和管理员访问；学生只能查看自己的尝试历史。

在提交记录下方填写“编写评语”并保存，学生即可在“学习中心 → 我的实验记录”中查看。
每次提交保留一条当前评语，记录最后修改的教师/管理员与时间；它不是评分，也不修改自动完成状态。
评语按纯文本显示，去除首尾空白后允许 1～2000 个字符，支持中文和多行内容。
如果其他教师已修改该评语，页面会拒绝覆盖并保留草稿；点击“加载最新评语”，核对后再手动保存。
保存失败也不会清空草稿。教学 API 仍受角色和 CSRF 来源检查保护。

本地历史采用共享快照和有界最近页选择，但保存时仍重写整份 JSON；已启动的本地保存会在超时/
断开后继续完成，重试前先刷新当前评语。应保留备份，不要将它视作
追加式持久日志；开销、限制及本机基准复现方法见[学习记录存储](learning-storage.md)。
