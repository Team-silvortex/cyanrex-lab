...
0.3.7 [测试网络](testing-network.md)最初发现 **9 个失败项**（认证 4、脚本持久化 3、事件删除 2）。
目前已修复，原有断言均通过；见独立的[修复验证](../../reports/acceptance/2026-09-09-boundary-fixes/result.json)。
下文保留历史失败证据，不改写成成功记录，也不代表现有部署已更新。
这些修复现收录于 **0.3.8**；报告仍保留升版前 **0.3.7** 的输入与哈希，不改写为 0.3.8 制品验收。
...
2026-09-09 对包含未提交变更的 0.3.7 工作区补测，未修改业务逻辑或现有部署。
新增 8 项 PostgreSQL 路径在三轮串行/并行运行中均为 **4 项通过、4 项失败**；此前 12 项认证
回归仍全部通过。新增 5 项课堂浏览器失败/取消场景通过，合并浏览器用例共 25 项通过；
浏览器使用模拟 Engine 响应，并非真实课堂前后端到数据库的端到端验收。

未修复问题，按优先级排列：

- **P1——账号重建隔离：** 旧凭据的登录在插入会话前暂停，期间同名账号被删除、重建，
  恢复后仍能获得该用户名的有效会话。
- **P2——登录/删除竞态：** 不重建账号时，进行中的登录仍返回 200 和无效 Cookie，
  外键拒绝还会触发认证持久化降级。
- **P2——会话级联删除被跳过：** 注入返回 `NULL` 的 `BEFORE DELETE` 触发器后，
  删账号报成功，却留下没有对应用户的有效持久会话。
- **P2——会话插入被跳过：** 注入返回 `NULL` 的 `BEFORE INSERT` 触发器后，登录报成功，
  但新会话无效。后两项是故障注入边界，不表示默认 schema 会自行跳过写入。

会话删除步骤报错、COMMIT 阶段延迟报错、两步删除之间取消请求，均正确回滚账号/会话删除。
课堂加入被取消后不会恢复邀请，但这些运行中的账号 INSERT 实际已提交：取消不代表回滚。
浏览器遇到断网、无效 JSON、503、不完整注册响应及离开页面时，均清除秘密且不自动重试。
这不表示中断注册的账号无需教师人工恢复。

复现在 `engine/src/services/auth_service/postgres_boundary_tests.rs`，选定源码哈希和输出摘录
位于 `reports/acceptance/2026-09-09-auth-boundaries/`。仅对**全新、可丢弃数据库**，使用
`cargo test --manifest-path engine/Cargo.toml --locked --lib` 选择
`services::auth_service::postgres_boundary_tests::`，并传入 `-- --ignored --nocapture`。
保持 `DATABASE_URL` 未设置，仅配置 `CYANREX_TEST_DATABASE_URL` 和 `CYANREX_DB_FALLBACK=true`。
新增测试尚未接入现有 CI 精确名称列表，因数据库隔离要求仍需显式运行；其中 4 项仍然失败，
常规默认门禁绿灯不能替代这些失败结果。临时数据库已删除，临时前端已停止；未操作现有部署
或加载内核程序，本检查点不构成发布验收。
