...
| Release traceability | Accepted unsigned candidates | Changelog/version sync, annotated `v0.2.9`/`v0.3.1` targets, immutable-Tag validation, checksum-bound source/image metadata, and exact-archive install acceptance before 30-day workflow retention; `0.3.0` is an API baseline only and signed publication remains manual |
