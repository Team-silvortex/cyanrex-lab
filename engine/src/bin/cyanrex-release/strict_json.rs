...
pub fn from_slice(source: &[u8]) -> Result<Value, String> {
...
mod tests {
    ...
    fn rejects_duplicate_keys_at_any_depth() {
        let error = from_slice(br#"{"outer":{"value":1,"value":2}}"#).unwrap_err();
        assert!(error.contains("duplicate key \"value\""), "{error}");
    }

    #[test]
    fn accepts_each_json_value_kind() {
        let value = from_slice(br#"{"a":[null,true,-1,2,3.5,"text"]}"#).unwrap();
        assert_eq!(value["a"][3], 2);
    }
}
